...
Suppose a misaligned system exists whose goals are not quite ours, and there is still an open gap between how capable it is and how ready we are to control it. The window this question is about opens at the AGI crossing, with the system barely past the line. It keeps growing through that window: median capability when a takeover resolves is about 56 % beyond the threshold (`future.md` §3.1), which is why the rest of this section calls the actor a superintelligence. Is the danger of a decisive takeover front-loaded, packed into the first years after the crossing and fading as time passes, or flat, staying just as high for as long as the gap is open?
